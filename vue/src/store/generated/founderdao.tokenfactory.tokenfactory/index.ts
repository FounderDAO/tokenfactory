import { Client, registry, MissingWalletError } from 'FounderDAO-tokenfactory-client-ts'

import { Denom } from "FounderDAO-tokenfactory-client-ts/founderdao.tokenfactory.tokenfactory/types"
import { Params } from "FounderDAO-tokenfactory-client-ts/founderdao.tokenfactory.tokenfactory/types"
...
export default {
	...
	actions: {
		init({ dispatch, rootGetters }) {
			console.log('Vuex module: founderdao.tokenfactory.tokenfactory initialized!')
		...
		async QueryParams({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				let value= (await client.FounderdaoTokenfactoryTokenfactory.query.queryParams()).data
		...
		async QueryDenom({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.FounderdaoTokenfactoryTokenfactory.query.queryDenom( key.denom)).data
		...
		async QueryDenomAll({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.FounderdaoTokenfactoryTokenfactory.query.queryDenomAll(query ?? undefined)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await client.FounderdaoTokenfactoryTokenfactory.query.queryDenomAll({...query ?? {}, 'pagination.key':(<any> value).pagination.next_key} as any)).data
		...
		async sendMsgUpdateDenom({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const result = await client.FounderdaoTokenfactoryTokenfactory.tx.sendMsgUpdateDenom({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
		...
		async sendMsgMintAndSendTokens({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const result = await client.FounderdaoTokenfactoryTokenfactory.tx.sendMsgMintAndSendTokens({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
		...
		async MsgUpdateDenom({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.FounderdaoTokenfactoryTokenfactory.tx.msgUpdateDenom({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgUpdateDenom:Init Could not initialize signing client. Wallet is required.')
		...
		async MsgMintAndSendTokens({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.FounderdaoTokenfactoryTokenfactory.tx.msgMintAndSendTokens({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgMintAndSendTokens:Init Could not initialize signing client. Wallet is required.')
